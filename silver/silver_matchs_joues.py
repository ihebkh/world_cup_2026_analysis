...
# ============================================================
# SILVER LAYER - Matchs Joues
# Lecture : ../bronze/matchs_joues  (parquet)
# Sortie  : ./matchs_joues          (parquet dans ce même dossier)
# ============================================================

spark = (
    SparkSession.builder
    .appName("Silver_Matchs_Joues")
    .getOrCreate()
)
...
df = spark.read.parquet(f"{BRONZE_PATH}/matchs_joues")
...
df_silver = (
    ...
    .withColumn("is_match_joue", F.lit(True))
    .withColumn("total_buts",
        F.col("score_domicile_final") + F.col("score_exterieur_final"))
    .withColumn("resultat_match",
        F.when(F.col("score_domicile_final") > F.col("score_exterieur_final"), F.lit("HOME_WIN"))
         .when(F.col("score_domicile_final") < F.col("score_exterieur_final"), F.lit("AWAY_WIN"))
         .otherwise(F.lit("DRAW")))
    .withColumn("vainqueur_equipe_id",
        F.when(F.col("score_domicile_final") > F.col("score_exterieur_final"), F.col("equipe_domicile_id"))
         .when(F.col("score_domicile_final") < F.col("score_exterieur_final"), F.col("equipe_exterieur_id"))
         .otherwise(F.lit(None).cast("long")))
    .withColumn("_silver_processed_timestamp", F.current_timestamp())
)

df_silver.write.mode("overwrite").parquet(f"{SILVER_PATH}/matchs_joues")

print("Silver OK : matchs_joues")
spark.stop()
